---
mode: 'agent'
description: 'Scaffold the complete ABP application layer for an existing entity — error codes, localization, validation, permissions, app service, and optional Razor Pages UI'
tools: ['codebase', 'fetch', 'search', 'editFiles']
---

You are an expert ABP Framework developer. Scaffold the **complete application feature layer** for an existing entity — wiring up all best practices: error codes, localization strings, FluentValidation, permissions, app service (with proper exception handling), and optionally Razor Pages UI.

If no entity name or entity file path was provided, ask for it before proceeding.
...
6. Fetch https://docs.abp.io/en/abp/latest/Application-Services for the latest API details.
...
- `<Entity>Consts` constants (`MaxXxxLength` values — must match DTO `[StringLength]`)
- Business methods (`SetName`, `SetPrice`, etc.) — call these in the app service, not direct assignment
...
Convention: `<Module>:<4-digit-number>`. Reserve a numeric block per entity for uniqueness.

---

## Step 3 — Localization strings

Add to `src/Acme.BookStore.Domain.Shared/Localization/BookStore/en.json` (`"texts"` object):
...
public class CreateUpdate<Entity>Dto
{
    ...
    // [Range(0.01, 9999.99)] for decimal ranges
...
### `src/Acme.BookStore.Application/<Entity>s/CreateUpdate<Entity>DtoValidator.cs`
...
> - `IRepository.GetAsync` throws `EntityNotFoundException` automatically → HTTP 404 — do NOT add a manual null-check
> - `<Entity>Manager.CreateAsync` throws `BusinessException` with an error code on duplicate — do NOT re-check uniqueness in the app service
> - Always call business methods (`SetName`, etc.) — never assign entity properties directly
> - `ObjectMapper.Map<>()` for entity↔DTO — entities never leave the domain layer

---

## Step 9 — AutoMapper entries
...
1. Add permission constants to `BookStorePermissions.cs` if the file already existed (merge, don't replace).
2. Add all new `en.json` keys to `ar.json` and other culture files.
3. Run `dotnet build` to verify no compilation errors and AutoMapper profile is complete.
4. Optionally scaffold Razor Pages UI → attach `#abp-razor-page.prompt.md`.
5. Optionally scaffold data seeding → attach `#abp-data-seed.prompt.md`.
